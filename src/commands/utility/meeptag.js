...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!interaction.channel.isThread()) {
            interaction.reply({ content: '/meeptag can only be used in threads.', ephemeral: true });
        } else {
            ...
            const messageCache = await parentChannel.messages.fetch({ limit: 1, cache: false, before: threadCreationId })
                .then(messages => messages.filter(message => message.author.bot === true && message.author.id === meeplebotId && message.interaction === null))
                .catch(error => console.error(error, "Thread creation message not found."))
            ...
            //find the emoji that represents the most popular game day choice
